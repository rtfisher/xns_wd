MODULE SYSTEMXNS
  ...
  CHARACTER(LEN=30) :: FILEEOS = 'wd_eos_helmholtz_resampled.dat'
!  CHARACTER(LEN=30) :: FILEEOS = 'resampled_eos/APR_resampled.dat'
  ...
  INTEGER :: MAXLOOP = 200                 ! Max Number of loops for convergence in XNSMAIN (1 = TOV solution)
...
! Outer radii in geometrized units, modified for WD
  REAL,PARAMETER :: RMAX = 6000.       ! Outer radius for the regular grid if stretch=.FALSE.
...
!  REAL :: RHOSURF = 1.0E-08             ! Density at which the surface is set
  ...
  ! Unit conversion factor for density: CGS to geometrized units
  ! In geometrized units (G = c = Msun = 1), the density unit is Msun/(G*Msun/c^2)^3
  ! = Msun/(solar gravitational radius)^3 = 1.99e33 g / (1.477 km)^3 = 6.176e17 g/cm^3
  REAL,PARAMETER :: RHO_CGS_TO_GEOM = 6.176e17  ! Conversion factor [g/cm^3]
  REAL,PARAMETER :: LENGTH_CGS_TO_GEOM = 6.772e-6  ! [geom units per cm]
  REAL,PARAMETER :: LENGTH_GEOM_TO_CGS = 1.4766e5  ! [cm per geom unit]
  REAL,PARAMETER :: PRESS_GEOM_TO_CGS = 5.548e38 ! [geom units per dyne/cm^2]

  ! Central density in geometrized units
  ! To convert from CGS density: set RHOINI = your_density_cgs / RHO_CGS_TO_GEOM
  ! Current value: 1.366e-3 * 6.176e17 = 8.44e14 g/cm^3 (typical NS central density)
!  REAL,PARAMETER :: RHOINI = 1.366e-3 ! Central density in the Jordan frame (beware scheme converges to QUCONV)
  REAL,PARAMETER :: RHOINI = 2.0e9 / RHO_CGS_TO_GEOM ! Central density of 1.0e9 g/cm^3 converted to geometrized units
...
!  CHARACTER(LEN=30) :: FILEEOS = 'resampled_eos/APR_resampled.dat'
  ...
  REAL           :: OMG = 1.e-4                 ! Central Rotation rate; non-rotating
!  REAL           :: OMG = 2.3e-5              ! Central Rotation rate; close to breakup for near-Mch WDs
  ...
  REAL,PARAMETER :: CUT = 1.0         ! Maximum distance for twisted magnetosphere
  ...
  LOGICAL :: DEBUG = .TRUE. 		! Set to true to print things for debugging purposes
