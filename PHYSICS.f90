...
SUBROUTINE GRIDBUILD(R,DR)
  ...
  ! Define the position of the medium radius of the domain (needed for mass convergence in TOVINI).
  IF(.NOT. STRETCH) THEN
     MIDGRID = NR/2.
  ELSE
...
!RTF: Convert pressure to rest-mass density by log-log interpolation
! from EOS table.
...
!RTF: Remove hardwired floor
    ...
    END IF
!RTF: Extrapolation above or below the limits of the EOS table
    IF(IL .LT. 0)THEN
